...
fn test_transfer() {
    new_test_ext().execute_with(|| {
        ...
        // Check kitty is there
        assert_eq!(KittiesModule::kitties(100, 0), Some(kitty.clone()));

        // Transfer bad ID, kitty index 1 does not exist
        let res = KittiesModule::transfer(Origin::signed(100), 0, 1);
        if let Err(DispatchError::Module{message, ..}) = res {
            assert_eq!("InvalidKittyId", message.unwrap());
        }

        // Transfer to self with a good ID
        assert_ok!(KittiesModule::transfer(Origin::signed(100), 100, 0));

        //Check that the kitty is still there
        assert_eq!(KittiesModule::kitties(100, 0), Some(kitty.clone()));

        // Transfer to self with a bad ID
        let res = KittiesModule::transfer(Origin::signed(100), 100, 1);
        if let Err(DispatchError::Module{message, ..}) = res {
            assert_eq!("InvalidKittyId", message.unwrap());
        }

        // Transfer to a different person with a good ID
        assert_ok!(KittiesModule::transfer(Origin::signed(100), 1, 0));
        // An Event is emitted
        assert_eq!(last_event(), Event::kitties(RawEvent::KittyTransferred(100, 1, 0)));
        //Check that the kitty is transferred
        assert_eq!(KittiesModule::kitties(100, 0), None);
        assert_eq!(KittiesModule::kitties(1, 0), Some(kitty.clone()));
    });
}